...
const TourInfo = (props: any) => {
    const { price, oldPrice, tripType, id } = props;
          ...
          <Row gutter={[16, 16]}>
            <Col xs={24} md={16}>
              <div className="flex items-center">
                ...
                <div className="font-semibold text-lg">{tripType}</div>
              </div>
              ...
              {/* Tabs */}
              <div className="mt-4 border border-gray-200 flex justify-center items-center rounded-lg overflow-x-auto">
                <Row gutter={[8, 8]} className="py-3 text-gray-600 flex-nowrap">
                  {["Overview", "Itinerary", "Details", "Reviews"].map(
                    (tab) => (
                      <Col key={tab}>
                        <Button
                          type="text"
                          className="text-gray-600 hover:text-black transition-colors duration-200 ease-in-out whitespace-nowrap"
                          onClick={() => {
              ...
              <div className="p-6 rounded-lg shadow-lg">
                <div className="flex flex-col justify-end gap-3 mb-4">
                  ...
                  <div className="text-right text-xl">
                    From{" "}
                    <span className="text-4xl text-green-600">${price}</span>
                  </div>
                </div>
                <Link href={`/order-tour/${id}`}>
